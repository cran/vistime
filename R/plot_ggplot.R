...
plot_ggplot <- function(data, linewidth, title, show_labels, background_lines) {
  ...
  if(show_labels){
    gg <- gg +
      ...
      ggrepel::geom_text_repel(mapping = aes_(colour = ~I(fontcol), label = ~label), data = event_dat, direction = "y", segment.alpha = 0, point.padding = grid::unit(0.75, "lines"))#, nudge_y = rep_len(c(0.3,-0.3), nrow(event_dat)))
  }
